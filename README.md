...
- Todo
    ...
    **ポイント**
    ...
    - [ ]  **`ArchiveView` のUI構築**:
        - [ ]  `List` ビューを使って、過去の `WeeklyRecord` を週ごとに表示する。
    - [ ]  **`DataManager` に取得ロジックを追加**:
        - [ ]  `WeeklyRecord` データを日付順に取得する処理を `DataManager` に追加。
    - [ ]  **`ArchiveView` でデータ表示**:
        - [ ]  `DataManager` を使って取得したデータを、`ArchiveView` でリスト表示する。
    - [ ]  **詳細表示機能**:
        - [ ]  リストの各項目をタップすると、その週の詳細な `WeeklyRecord` を表示する機能を実装する。
    
    ### フェーズ5: データ永続化のテスト (2-3日)
    ...
    ## スプリント4: アーカイブ機能とユーティリティ (1-2週間)
    
    - [ ]  `ArchiveView`の実装
        - [ ]  過去の記録リスト表示
        - [ ]  詳細表示機能
    - [ ]  `DateFormatter`ユーティリティの作成
    - [ ]  `Constants`ファイルの作成と共通値の定義
    - [ ]  `HomeView`の更新
        - [ ]  アーカイブへのナビゲーション追加
    - [ ]  アーカイブ機能のテスト
    
    ## スプリント5: 設定機能とテストの追加 (1-2週間)
